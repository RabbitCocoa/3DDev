...
static const int BAKE_MODE_FULL = 0;
static const int BAKE_MODE_INDIRECT = 1;
...
float3 Raytracing(in const float3 wspos, in const float3 N, inout Random rng)
{
    ...
    for (int i = 0; i < BounceCount; ++i)
    {
        ...
        // sample light
        if(_LightmapBakeMode == BAKE_MODE_FULL  || i >0)
        {
        ...
        {
            radiance += _TexSkyLight.SampleLevel(s_linear_clamp_sampler, bounce.ray.Direction, 0) * _SkyLightIntensity *
                bounce.throughput;
            break;
        }
    }
